...
import { Shield, LayoutDashboard, Search, AlertCircle, Cloud, Server, FileText, Users, Settings, Bell, Calendar, Menu, X, Bot, Sparkles, Send, LogOut } from 'lucide-react';
...
export default function DashboardLayout() {
  ...
  const displayName = currentUser?.fullName || currentUser?.email || 'Guest User';
  const displayRole = currentUser?.email ? (currentUser.email.includes('admin') ? 'SOC Admin' : 'SOC Analyst') : 'Guest Analyst';
  ...
  return (
    <div className="flex h-screen bg-navy-900 overflow-hidden text-slate-200">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-navy-800 border-r border-slate-800 flex flex-col transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 border-t border-slate-800 bg-navy-900/60 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-white truncate" title={displayName}>{displayName}</p>
              <p className="text-[10px] text-slate-400 truncate">{displayRole}</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="p-1.5 text-slate-500 hover:text-status-critical hover:bg-slate-800/60 rounded-lg transition-all"
            title="Log Out"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 border-b border-slate-800 bg-navy-900/90 backdrop-blur-md z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="h-8 w-8 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center text-primary font-bold text-xs">{initials}</div>
          </div>
        </header>
