...
export default function UserProfilePanel({
  ...
  const storedUser = storage.getUser() as { name?: string; fullName?: string; email?: string; role?: string } | null;
  const effectiveName = storedUser?.name || storedUser?.fullName || (storedUser?.email ? storedUser.email.split('@')[0] : userName);
  const effectiveRole = storedUser?.role ? `${storedUser.role} Console` : userRole;
  ...
  const initials = (effectiveName.includes(' ')
    ? effectiveName.split(' ').map((n) => n[0]).join('')
    : effectiveName.substring(0, 2)
