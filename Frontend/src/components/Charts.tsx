...
export function DonutChart({ segments, centerLabel = 'assets' }: DonutChartProps) {
  const { colors } = useTheme();
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, flexWrap: 'wrap' }}>
      <svg width={180} height={180} viewBox="0 0 180 180" role="img" aria-label={`${centerLabel} distribution`}>
        ...
        <text x="90" y="102" textAnchor="middle" fill={colors.secondaryText} fontSize="12">{centerLabel}</text>
      </svg>
      <Box>
        {segments.map((segment) => (
