...
## Milestone 2: Asset and Vulnerability Management

The frontend uses one Axios client with the JWT request interceptor in
`src/services/api.ts`. Asset and vulnerability requests are protected by the
backend's authenticated API policy.

Vulnerability management is available at `/threat-detection` and includes:

- Real vulnerability records from `GET /api/vulnerabilities`
- Asset, severity, status, and text search filters
- Create, edit, status update, delete, and detail workflows
- Loading, empty, validation, API error, and success states
- Asset Details vulnerability counts and related findings
- Dashboard severity, status, and provider distributions from API data

Vulnerability detail fields are sourced from the backend DTO only. The client
does not send or edit `id`, `createdAt`, `updatedAt`, or `resolvedAt`.
...
| GET | `/api/vulnerabilities/{id}` | View one vulnerability |
| GET | `/api/vulnerabilities/asset/{assetId}` | List findings for an asset |
| GET | `/api/vulnerabilities/severity/{severity}` | Filter by severity |
| GET | `/api/vulnerabilities/status/{status}` | Filter by status |
| POST | `/api/vulnerabilities` | Create a finding |
| PUT | `/api/vulnerabilities/{id}` | Update editable fields |
| PATCH | `/api/vulnerabilities/{id}/status` | Change lifecycle status |
| DELETE | `/api/vulnerabilities/{id}` | Delete a finding |

The backend relationship is `Asset 1:N Vulnerability`. Duplicate findings are
blocked per asset, vulnerability identifier, and affected component. Statuses
are `OPEN`, `IN_PROGRESS`, `RESOLVED`, and `ACCEPTED`; the backend controls
`resolvedAt`.
